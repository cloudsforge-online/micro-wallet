...
# below SCHEMA_VERSION the unique constraint on `deposit_credits.credit_key` may not exist, and
# that constraint is one of the two things stopping a redelivered deposit crediting twice. A
# service that could create it at boot is a service that could start without it. `index.ts`
